function [model, A_final] = fastcormics(model, data, rownames, dico, biomass_rxn, already_mapped_tag, consensus_proportion, epsilon, optional_settings)
...
model = fixIrr_fastcormics(model);

%fix rules
if ~isfield(model, 'rules')
    disp('creating model.rules')
    model = generateRules_fastcormics(model);
end
...
if already_mapped_tag~=1
    mapping = map_expression_2_data_fastcormics(model, data, dico, rownames);
...
if ~isempty(optional_settings) && isfield(optional_settings, 'medium')
    ...
    model =  constrain_model_fastcormics(model, medium_mets, not_medium_constrained, biomass_rxn, optional_settings.func);
else
