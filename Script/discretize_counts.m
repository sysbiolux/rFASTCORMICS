function [discretized] = discretize_FPKM(counts,colnames,figflag)
%% Discretize_Data
% script adapted from (c) Dr. Maria Pires Pacheco 2016
load('P:\PhD Project\Resources\Models\Recon2_04\Recon204.mat','CmodelR204')
...
for j=1:size(counts,2) %for each sample
...
%     j
    ...
    if figflag; figure
        ...
        ylabel('abundance');xlabel('log2(FPKM)');title({'Histogram of abundance',['Sample: ', colnames{j}]},'Interpreter','none');
                saveas(gcf,['Figures\histogram_', colnames{j},'.png'])
    end
    ...
    if figflag;figure; hold on
        ...
        ylabel('Density'); xlabel('log2(Counts)');
    ...
    pks = findpeaks( probability_estimate )
    pks = (pks((pks > max(pks)*0.5)))
    peak_max_idx = max(find(ismember(probability_estimate, pks)))

%     [pks,locs,w,p] = findpeaks(probability_estimate);
%     peak_max_idx = max(locs);
...
%     hybrid = [max_l_side(1:end-1), max_r_side];
    ...
    %         saveas(gcf,['Figures\zFPKM_', colnames{j},'.png']);close all;end
    ...
    if figflag; figure;hold on;
        ...
        title({'Expression thresholds',['Sample: ', colnames{j}],['inexpression threshold = ', num2str(ue)]},'Interpreter','none');
                ...
                saveas(gcf,['Figures\zFPKM_', colnames{j},'.png']);
                close all;
    end
...
if figflag;figure; hold on
    ...
    %     legend(colnames,'interpreter','none','Location','best')
        saveas(gcf,['Figures\density_all.png']);close all
end
...
if figflag;figure; hold on
    for j=1:length(colnames) %for each sample
        ...
        saveas(gcf,['Figures\density_subplots.png']);close all
end
